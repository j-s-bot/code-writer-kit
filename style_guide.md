...
2. **Don't Repeat Yourself (DRY) & Cross-Module Sharing**:
   ...
   - If a constant, identifier, template name, schema, or logic snippet is referenced in multiple modules (such as prompt names, shared graph node IDs, or domain models), move it to a single authoritative shared file (e.g., `src/prompts/names.py`, `src/graphs/common.py`) and import it.
...
7. **Avoid Magic Numbers, Constants & Module-Level Instances**:
